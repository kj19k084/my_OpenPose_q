...
def draw_person_pose(orig_img, poses, file_name): ##ファイル名も引数に追加
    if len(poses) == 0:
        return orig_img
    ...
    for pose in poses.round().astype('i'):
        ...
        for i, ((x, y, v), color) in enumerate(zip(pose, joint_colors)):
            if v != 0:
                ...
                joint_points.append([j, file_name, i, x, y]) ##

    # output csv ##
    df = pd.DataFrame(joint_points)
    df.to_csv("result/poseposi.csv", mode='a', header=False) ##
...
if __name__ == '__main__':
    ...
    # draw and save image
    img = draw_person_pose(img, poses, str(args.img))
    print('Saving result into result.png...')
    filename = 'result/result_' + os.path.basename(args.img)
